...
def visualize_value_iteration(grid, rewards, start_state, goal_states, transition_probs, env):
    # visualize
    import numpy as np
    ...
    result = solve_mdp(env, method='value_iteration', gamma=0.9, theta=1e-5)
    policy = result['policy']
    state_values = result['state_values']

    render_policy_and_values(grid, state_values, policy)
    plot_policy_and_values(grid, policy, state_values, title="Value Iteration Result")
